...
const TrainerCard = forwardRef(({ profile, hideDownload }, ref) => {
  ...
  // --- SILENT IMAGE LOADER (Fixes console errors & download crashes) ---
  ...
  useEffect(() => {
    ...
    const loadImages = async () => {
      const neededHeroes = [
        profile.main_hero_1, profile.main_hero_2, profile.main_hero_3, 
        profile.hate_hero, profile.love_hero
      ].filter(Boolean);

      const fetched = {};
      
      for (const name of neededHeroes) {
        ...
        try {
          ...
          // Silent fail - prevents the console error you keep seeing
        }
      }
  ...
  // Returns the safe Base64 string, or null (which triggers the text fallback)
    ...
    >
      ...
      <div className="w-[38%] bg-tavern-accent p-6 flex flex-col justify-center items-center text-center relative overflow-hidden">
        ...
        <div className="relative z-10 w-full px-2">
          ...
          {profile.description && (
            <p className="mt-4 text-[10px] text-white/60 leading-snug italic line-clamp-4">
              "{profile.description}"
            </p>
          )}
        </div>
      ...
      <div className="w-[62%] p-5 flex flex-col justify-between bg-gray-900 relative">
        ...
        <img 
          src={mlbbLogo} 
          alt="MLBB Logo" 
          className="absolute top-4 right-4 h-4 w-auto object-contain opacity-40 brightness-0 invert pointer-events-none"
        />
              ...
              {heroes.length > 0 ? heroes.map((hero, i) => {
                return (
                  <div key={i} className="w-10 h-10 rounded bg-gray-800 border border-gray-700 overflow-hidden shrink-0 flex items-center justify-center">
                    {getSafeSrc(hero) ? (
                      <img src={getSafeSrc(hero)} alt={hero} className="w-full h-full object-cover" />
                    ) : (
                      <span className="text-[8px] text-gray-500 text-center leading-tight px-0.5">{hero}</span>
                    )}
                  </div>
                );
              }) : (
        ...
        <div className="mt-auto border-t border-gray-700 pt-2 flex justify-between items-end">
          <div>
            ...
            <p className="text-xs font-bold text-gray-300">ml-tavern.vercel.app</p>
          </div>
          <div className="bg-white p-1 rounded-sm">
            <QRCodeCanvas value="https://ml-tavern.vercel.app" size={36} bgColor="#ffffff" fgColor="#000000" level="L" />
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className={`border ${border} rounded p-1.5 flex items-center gap-2 bg-black/30`}>
      <div className="w-7 h-7 rounded bg-gray-800 border border-gray-700 overflow-hidden shrink-0 flex items-center justify-center">
        {getSafeSrc(name) ? (
          <img src={getSafeSrc(name)} alt={name} className="w-full h-full object-cover" />
        ) : (
