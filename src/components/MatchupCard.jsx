// ============================================================
// FILE: src/components/MatchupCard.jsx
// ============================================================
...
const getProxyUrl = (url) =>
  `https://images.weserv.nl/?url=${encodeURIComponent(url)}&w=120&h=120&fit=cover&t=square`;
const getHeroImg = (name) => {
  if (!Array.isArray(heroData) || !name) return null;
  return (
    heroData.find(
      (h) => h.hero_name?.toLowerCase() === name.toLowerCase()
    )?.portrait || null
  );
};
...
function HeroPortrait({ name, role, borderColor, accentColor }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      {!imgUrl || err ? (
        <div
          className={`w-10 h-10 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-md border-2 ${borderColor} bg-gray-900 flex items-center justify-center text-[10px] sm:text-xs text-white/30 font-bold`}
        >
          {name?.substring(0, 2)}
        </div>
      ) : (
...
const extractScores = (rawText) => {
  ...
  return {
    yourScore: parseInt(match[1]),
    enemyScore: parseInt(match[2]),
  };
};
  ...
  return (
    ...
    >
      <div className="bg-gradient-to-b from-gray-900 via-black to-gray-950 px-4 sm:px-8 py-6 sm:py-10 text-center border-b border-gray-800">
        <p className="text-[9px] sm:text-[11px] text-gray-500 uppercase tracking-[0.3em] sm:tracking-[0.4em] mb-2 sm:mb-3">
          Match Prediction
        </p>
        <h2
        ...
        {(yourScore > 0 || enemyScore > 0) && (
          <p className="mt-3 sm:mt-4 text-xl sm:text-2xl md:text-3xl font-black text-white/80 tracking-widest tabular-nums">
            {yourScore} <span className="text-white/30 mx-1">-</span>{' '}
            {enemyScore}
          </p>
        )}
      </div>
          ...
          <div className="flex items-center justify-center py-1 md:py-0 md:-my-4">
            <div className="flex md:hidden w-full items-center">
              ...
              <div className="w-12 h-12 sm:w-14 sm:h-14 mx-3 rounded-md border-2 border-white/20 bg-gray-900 flex items-center justify-center shadow-lg shadow-black/50 shrink-0">
                <span className="text-lg sm:text-xl font-black text-white/60 tracking-wider">
                  VS
                </span>
              </div>
              <div className="flex-1 h-px bg-white/10" />
            ...
            <div className="hidden md:flex flex-col items-center">
              ...
              <div className="w-16 h-16 rounded-md border-2 border-white/20 bg-gray-900 flex items-center justify-center shadow-lg shadow-black/50">
                <span className="text-2xl font-black text-white/60 tracking-wider">
                  VS
                </span>
              </div>
              <div className="w-px h-8 bg-white/10" />
      ...
      <div className="px-4 sm:px-8 py-3 sm:py-5 border-t border-gray-800 bg-black/50 flex justify-between items-center">
        ...
        <p className="text-[9px] sm:text-[10px] font-bold text-white/30">
          mlbb-tavern.site
        </p>
      </div>
    </div>
