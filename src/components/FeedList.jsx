...
function CreatePostModal({ open, onClose, preselectedTag, onCreated }) {
  const { user } = useAuth();
  ...
  useEffect(() => { if (open) { setTag(preselectedTag || 'Pilot Service'); setTitle(''); setContent(''); setError(''); } }, [open, preselectedTag]);
  ...
  const handleSubmit = async (e) => {
    ...
    const { error: err } = await supabase.from(TAG_TO_TABLE[tag]).insert({
      user_id: user.id, title: title.trim(), content: content.trim(),
    });
    if (err) { setError(err.message); setSubmitting(false); return; }
    setSubmitting(false); onClose(); onCreated?.();
  };
...
export default function FeedList({ tagFilter = null, title = 'Feed', showCreateButton = true }) {
  ...
  return (
    <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-3">
            {visible.map(p => (
              <PostCard key={`${p.board_type}-${p.id}`} post={p}
                onUpvoteChange={handleUpvoteChange} onBookmarkChange={handleBookmarkChange} onPinChange={handlePinChange} />
            ))}
          </div>
          {hasMore && (
      ...
      <CreatePostModal open={showCreate} onClose={() => setShowCreate(false)} preselectedTag={tagFilter} onCreated={fetchPosts} />
    </div>
  );
}